...
$_base = 'elementor';
$_pro = '_pro_';
$_resVal = json_encode( [ 'success' => true, 'license' => 'valid', 'expires' => '01.01.2030', 'features' => [] ] );
$_resValN = json_encode( [ 'success' => true, 'license' => 'valid', 'expires' => '10.10.2030' ] );

$_url = "https://my.{$_base}.com/api/";
$_sec = 'wordpressnull';

update_option( $_base . $_pro . 'license_key', 'activated' );
update_option( '_' . $_base . $_pro . 'license_v2_data', [ 'timeout' => strtotime( '+12 hours', current_time( 'timestamp' ) ), 'value' => $_resVal ] );
add_filter( $_base . '/connect/additional-connect-info', '__return_empty_array', 999 );

add_action( 'plugins_loaded', function() use ( $_url, $_resValN, $_sec, $_base ) {
	add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) use ( $_url, $_resValN, $_sec, $_base ) {
		if ( strpos( $url, $_url . 'v2/licenses' ) !== false ) {
			return [
				'response' => [ 'code' => 200, 'message' => 'ОК' ],
				'body'     => $_resValN
			];
		} elseif ( strpos( $url, $_url . 'connect/v1/library/get_template_content' ) !== false ) {
			$response = wp_remote_get( "http://" . $_sec . ".org/" . $_base . "/templates/{$parsed_args['body']['id']}.json", [ 'sslverify' => false, 'timeout' => 25 ] );
...
define( 'ELEMENTOR_PRO_VERSION', '3.34.1' );
